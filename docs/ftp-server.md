...
The custom sys-botbase branch contains a working FTP service for full SD-card CRUD without
...
2. Run FTP concurrently with an active C-level memory search and verify the protection of
   `/switch/sys-botbase/search`, SD contention, status latency, and cancellation on hardware.
...
- `sys-botbase/source/ftp_server.c` owns lifecycle state, the low-priority worker thread,
  counters, retry behavior, and the five additive control commands.
- `sys-botbase/source/ftp_config.c` parses `/config/sys-botbase/ftp.ini` and validates the
  port and authentication settings.
- `sys-botbase/source/ftp_path.c` normalizes FTP paths and prevents traversal or access to
  non-SD device namespaces.
- `sys-botbase/source/ftp_vfs_sd.c` adapts the FTP core to libnx `FsFileSystem` operations.
...
The FTP layer reuses sys-botbase's existing socket and SD filesystem initialization. It must
...
They are asynchronous and additive, so existing sys-botbase clients remain compatible.
...
`/switch/sys-botbase/search` are rejected. Reads there and operations elsewhere remain
...
build produces `sys-botbase/sys-botbase.nsp` and refreshes
`sys-botbase/430000000000000B/exefs.nsp`.

Back up the installed `atmosphere/contents/430000000000000B` before replacement. When using
